...
export function WorkspaceControlTile({ id, name, created_at, updated_at, currentWorkspaceId }) {
                    ...
                    <div className="flex-2">
                        {
                            ...
                            {isEditing && (
                                ...
                                {!isEditing && (
                                    <Button onClick={() => setIsEditing(true)} type="button" className="select-none w-full">
                                        Rename
                                    </Button>
                                )}
                            </>
            ...
            <div className="mt-5">
                ...
                {(currentWorkspaceId === id) && (
                    <p className="text-xs select-none text-justify text-muted-foreground mt-2">
                        Note: You can't delete this workspace as it is currently active. Please switch to another workspace to proceed with deletion.
                    </p>
                )}

                {/* Delete Consent Dialog */}
                {(currentWorkspaceId !== id) && <DeleteWorkspaceButton workspaceId={id} />}
            </div>
        </div>
    );
}
